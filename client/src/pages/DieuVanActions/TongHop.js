    ...
    `}
      ...
      <div className="flex flex-wrap gap-2 mb-3 items-center w-full justify-start">
        ...
        <input
          ...
          onChange={(e) =>
            setDate(e.target.value ? new Date(e.target.value) : "")
          }
          className="border rounded px-3 py-2"
        />
      ...
      <div className="overflow-auto max-h-[75vh] border bg-white">
        <table className="border-collapse text-sm w-max">
          ...
          <tbody>
            {rides
              .filter((r) => {
                // ---- LỌC KHÁCH HÀNG KHÔNG DẤU ----
                if (filters.khachHang.trim()) {
                  const input = removeVietnamese(
                    filters.khachHang.toLowerCase().trim()
                  );
                  const name = removeVietnamese(
                    (r.khachHang || "").toLowerCase().trim()
                  );
