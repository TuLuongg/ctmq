...
export default function ManageAllTrip({ user, onLogout }) {
  ...
  const canSeeFinancialColumns = currentUser.permissions?.includes("edit_trip_full");
...
const exportToExcel = () => {
  ...
  const exportColumns = allColumns.filter(
    col =>
      ...
      (canSeeFinancialColumns || !financialColumns.includes(col.key))
  );
...
>
Yêu cầu cần duyệt {pendingCount > 0 && `(${pendingCount})`}
</button>
        <button
  ...
  >
    ...
    `: ${selectedTrips
      ...
      <div className="flex flex-wrap gap-2 mb-3 border p-2 bg-white rounded">
        {allColumns.filter(col => canSeeFinancialColumns || !financialColumns.includes(col.key)).map((col) => (
...
{allColumns
    ...
    (c) =>
      ...
      (canSeeFinancialColumns || !financialColumns.includes(c.key))
  )
      .map((col) => (
        <th
          key={col.key}
              ...
              <tr key={r._id} className="text-center">
                <td className="border p-2">
                  <button
                    ...
                    className="px-1 py-1 bg-yellow-400 text-white rounded hover:bg-blue-600"
                  >
                    <FaEdit />
                  </button>
                </td>
