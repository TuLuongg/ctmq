            ...
            {customers.map((c, idx) => {
              ...
              return (
                <tr key={c._id} onClick={() => toggleRowHighlight(c._id)} className={`cursor-pointer ${isWarning ? "bg-red-300" : idx % 2 === 0 ? "bg-white" : "bg-gray-50"} ${selectedRows.includes(c._id) ? "bg-yellow-200" : ""}`}>
                  {/* Cột cảnh báo */}
                  <td className="border p-1 text-center" style={{ position: "sticky", left: 0, zIndex: 40, width: 30, background: isWarning ? "#fca5a5" : "#fff" }}>
                  ...
                  {visibleColumns.map((cKey, colIndex) => {
                    ...
                    const stickyLeft = isFirst ? 30 : isSecond ? 30 + firstColWidth : undefined;
