...
export default function VoucherListPage() {
  ...
  <button onClick={handleGoToVouchers} className={`px-3 py-1 rounded text-white ${isActive("/voucher-list") ? "bg-green-600" : "bg-blue-500"}`}>Sổ phiếu chi</button>
      ...
      <div className="flex justify-between items-center mb-4"> <h1 className="text-lg font-bold">SỔ PHIẾU CHI</h1> </div>
      {/* Bộ lọc tháng/năm */}
      <div className="mb-4">
        <select value={month} onChange={(e) => setMonth(Number(e.target.value))} className="border px-2 py-1 rounded mr-2">
          {[...Array(12)].map((_, i) => <option key={i} value={i + 1}>Tháng {i + 1}</option>)}
        </select>
        <select value={year} onChange={(e) => setYear(Number(e.target.value))} className="border px-2 py-1 rounded mr-2">
          {[2023, 2024, 2025, 2026].map(y => <option key={y} value={y}>Năm {y}</option>)}
        </select>
        <button onClick={() => { setMonth(now.getMonth() + 1); setYear(now.getFullYear()); }} className="px-3 py-1 rounded bg-blue-500 text-white mr-2">RESET</button>
        <button onClick={() => setShowCreate(true)} className="px-3 py-1 rounded bg-green-600 text-white">Tạo phiếu mới</button>
      </div>
      ...
      <div className="overflow-auto mb-6">
        <table className="w-full text-sm">
          ...
          <tbody>
            {loading ? (
              <tr><td colSpan="9" className="p-4 text-center">Đang tải...</td></tr>
            ) : vouchersOriginal.length === 0 ? (
              <tr><td colSpan="9" className="p-4 text-center">Không có phiếu gốc</td></tr>
            ) : (
              vouchersOriginal.map((v, idx) => (
                <tr key={v._id} className="hover:bg-gray-50">
                  <td className="border p-2">{idx + 1}</td>
                  <td className="border p-2">{new Date(v.dateCreated).toLocaleDateString("vi-VN")}</td>
                  <td className="border p-2">{v.paymentSource === "caNhan" ? "Cá nhân" : "Công ty"}</td>
                  <td className="border p-2">{v.receiverName}</td>
                  <td className="border p-2">{v.receiverCompany}</td>
                  <td className="border p-2">{v.transferContent || v.reason}</td>
                  <td className="border p-2 text-right">{v.amount?.toLocaleString()}</td>
                  <td className="border p-2 text-center font-semibold">
                    {v.status === "waiting_check" && <span className="text-yellow-600">Đang chờ duyệt</span>}
                    {v.status === "approved" && <span className="text-green-600">Đã duyệt</span>}
                  </td>
                  <td className="border p-2">
                    <div className="flex justify-center items-center gap-2">
                      <button className="text-blue-600" onClick={() => setDetailId(v._id)}>Xem</button>
                      <button className="text-red-600" onClick={() => window.open(`/voucher/${v._id}/print`)}>In phiếu</button>
                    </div>
                  </td>
                </tr>
              ))
      ...
      <div className="overflow-auto">
        <table className="w-full text-sm">
          ...
          <tbody>
            {loading ? (
              <tr><td colSpan="10" className="p-4 text-center">Đang tải...</td></tr>
            ) : vouchersAdjusted.length === 0 ? (
              <tr><td colSpan="10" className="p-4 text-center">Không có phiếu điều chỉnh</td></tr>
            ) : (
              vouchersAdjusted.map((v, idx) => {
                ...
                return (
                  <tr key={v._id} className="hover:bg-gray-50">
                    ...
                    <td className="border p-2">
                      {orig ? (
                        <button className="text-blue-600 underline" onClick={() => setShowOrigDetail(orig._id)}>
                          {orig._id.slice(-6)} {/* hiển thị 6 ký tự cuối làm tham chiếu */}
                        </button>
                      ) : "-"}
                    </td>
                    <td className="border p-2 text-center font-semibold">
      ...
      {/* Modals */}
      {showCreate && <VoucherCreateModal customers={customers} onClose={() => setShowCreate(false)} onSuccess={() => { setShowCreate(false); load(); }} />}
      {detailId && <VoucherDetailModal id={detailId} customers={customers} onClose={() => { setDetailId(null); load(); }} />}
      {showOrigDetail && <VoucherDetailModal id={showOrigDetail} customers={customers} onClose={() => setShowOrigDetail(null)} />}
    </div>
  );
}
